...
void initialize_outgoing_data(int control_mode, int motor_speed){
  outgoing_data[0] = control_mode;   // control mode - 1: mode speed, 2: mode angle, 3: mode speed angle
...
void set_outgoing_data(float el_dot_desired, float az_dot_desired){
  int16_t speed_pitch = el_dot_desired * SBGC_SPEED_SCALE;  
  outgoing_data[5] = speed_pitch;  // speed pitch
  outgoing_data[6] = speed_pitch >> 8;
  int16_t speed_yaw = az_dot_desired * SBGC_SPEED_SCALE;
  outgoing_data[9] = speed_yaw;  // angle yaw
  outgoing_data[10] = speed_yaw >> 8;
}
...
//  Serial.println("desired");
  ...
  float az_dot_d;
  Serial.readBytes((char*)&az_dot_d, sizeof(az_dot_d));

  // reading the desired elevation angle
  while(!Serial.available());
  float el_dot_d;
  Serial.readBytes((char*)&el_dot_d, sizeof(el_dot_d));
  
  Serial.println(az_dot_d);
  Serial.println(el_dot_d);
  
  // move the motors to the desired position
  set_outgoing_data(-el_dot_d, az_dot_d);
  SBGC_sendCommand(67, outgoing_data, 13);
